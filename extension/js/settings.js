...
// DOM elements - will be initialized after the DOM is loaded
let apiKeyInput;
let showApiKeyButton;
let apiKeyStatus;
let blacklistInput;
let delaySlider;
let delayValue;
...
function initialize() {
    ...
    if (!apiKeyInput) console.error("apiKeyInput not found");
    if (!showApiKeyButton) console.error("showApiKeyButton not found");
    if (!apiKeyStatus) console.error("apiKeyStatus not found");
    if (!blacklistInput) console.error("blacklistInput not found");
    if (!delaySlider) console.error("delaySlider not found");
    if (!delayValue) console.error("delayValue not found");
    if (!saveButton) console.error("saveButton not found");
    if (!backButton) console.error("backButton not found");
    ...
    if (apiKey) {
        ...
        try {
            ...
            apiKeyStatus.textContent =
                error.message || "Error validating API key";
            apiKeyStatus.className = "status-message error";
